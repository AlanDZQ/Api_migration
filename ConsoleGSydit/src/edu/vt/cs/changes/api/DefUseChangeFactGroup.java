...
//				fact.compare_two_patterns(templateLeft, templateRight);				
				left_pre_output = fact.renderWithTypeInfo(templateLeft);
...
//				fact.compare_two_patterns(templateLeft, templateRight);
				right_pre_output = fact.renderWithTypeInfo(templateRight);			
			}
...
//		if (!oldTemplate.equals("") && !newTemplate.equals("")) refined_label = false;
		...
		if (refined_label) {
			if (purecheck(oldTemplate) && purecheck(newTemplate) && !CommonValue.common_old_version.equals(CommonValue.common_new_version)) {
				...
				int label = data1.insertpattern(oldTemplate, newTemplate, CommonValue.common_old_version, CommonValue.common_new_version, "Group Changes", lv_label);
